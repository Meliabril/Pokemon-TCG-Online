...
class CoinFlipUntilTailsDamageAttackEffectTest {
    ...
    @Test
    void shouldSumDamagePerHeadsUntilFirstTails() {
        CoinFlipUntilTailsDamageAttackEffect effect =
                new CoinFlipUntilTailsDamageAttackEffect(gameRandomService, gameEventFactory);
        when(gameRandomService.flipCoin()).thenReturn(true, true, false);
        when(gameEventFactory.publicEvent(any(), eq(GameEventType.ATTACK_EFFECT_RESOLVED), any(Integer.class), any()))
                .thenReturn(event());

        AttackEffectResult result = effect.apply(context(operation(20)));
    ...
    @Test
    void shouldYieldZeroDamageWhenFirstFlipIsTails() {
        CoinFlipUntilTailsDamageAttackEffect effect =
                new CoinFlipUntilTailsDamageAttackEffect(gameRandomService, gameEventFactory);
        ...
        AttackEffectResult result = effect.apply(context(operation(30)));
    ...
    void shouldIncludeActorAndPokemonIdentifiersInEventPayload() {
        ...
        AttackEffectContext context = context(operation(20));
    ...
    private AttackEffectOperation operation(int amount) {
        return new AttackEffectOperation(
                "COIN_FLIP_UNTIL_TAILS_DAMAGE",
