...
class EnergyPerTurnValidatorTest {

    @Mock
    private GameActionContext context;

    private final EnergyPerTurnValidator validator = new EnergyPerTurnValidator();

    @Test
    void shouldAllowEnergyAttachmentWhenNoEnergyWasAttachedThisTurn() {
        configureContext(GameActionType.ATTACH_ENERGY, state(false, false, false));

        validator.validate(context);
    }

    @Test
    void shouldAllowNonEnergyActionWhenEnergyWasAttachedThisTurn() {
        configureContext(GameActionType.PLAY_BASIC_POKEMON, state(true, false, false));

        validator.validate(context);
    }

    @Test
    void shouldRejectEnergyAttachmentWhenEnergyWasAlreadyAttachedThisTurn() {
        configureContext(GameActionType.ATTACH_ENERGY, state(true, false, false));

        assertThatThrownBy(validationCall())
                .isInstanceOf(InvalidGameActionException.class)
                .hasMessage("Energy already attached this turn");
    }
    ...
    private void configureContext(GameActionType actionType, GameStateDto state) {
